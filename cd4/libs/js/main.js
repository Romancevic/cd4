...
function watchForSelectedOption($arg) {
    let selectedDepartmentName = $('#removeDepartmentDepartment').val()
    if ($arg == "department") {
        $.getJSON(`libs/php/getAllDepartments.php`, function (departments) {
            let departmentID = departments.data.filter(dep => dep.name == selectedDepartmentName)[0].id
            $.ajax({
                url: "libs/php/getAffectedRecords.php",
                dataType: "text",
                method: "POST",
                data: {
                    'type': 'department',
    ...
    if ($arg == "location") {
        let selectedLocationName = $('#removeLocationLocation').val()
        $.getJSON(`libs/php/getAllLocations.php`, function (locations) {
            let locationID = locations.data.filter(dep => dep.name == selectedLocationName)[0].id
            $.ajax({
                url: "libs/php/getAffectedRecords.php",
                dataType: "text",
                method: "POST",
